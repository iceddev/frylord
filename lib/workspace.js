'use strict';

var fs = require('fs');
var path = require('path');

var del = require('del');
var mkdirp = require('mkdirp');
var Immstruct = require('immstruct').Immstruct;

var temp = require('./tempfile');

var root = process.browser ? '/' : './';

function Workspace(){
  var self = this;

  var inst = this._inst = new Immstruct();

  var structure = this._structure = inst.get({
    filename: '',
    current: 'function helloWorld(hello){\n  hello = "world";\n}',
    cwd: root,
    directory: [],
    projects: []
  });

  this.filename = structure.cursor('filename');
  this.current = structure.cursor('current');
  this.cwd = structure.cursor('cwd');
  this.directory = structure.cursor('directory');
  this.projects = structure.cursor('projects');

  // TODO: this should probably be tested
  structure.on('swap', function(n, o, updatedPath){
    if(updatedPath.indexOf('filename') === 0){
      self.filename = structure.cursor('filename');
      return;
    }

    if(updatedPath.indexOf('current') === 0){
      self.current = structure.cursor('current');
      return;
    }

    if(updatedPath.indexOf('cwd') === 0){
      self.cwd = structure.cursor('cwd');
      return;
    }

    if(updatedPath.indexOf('directory') === 0){
      self.directory = structure.cursor('directory');
      return;
    }

    if(updatedPath.indexOf('projects') === 0){
      self.projects = structure.cursor('projects');
      return;
    }
  });
}

Workspace.prototype.loadFile = function loadFile(filepath, cb){
  var self = this;

  var cwd = this.cwd.deref();
  var resolvedFilepath = path.resolve(cwd, filepath);

  function handleContents(err, contents){
    self.filename.update(function(){
      return filepath;
    });
    self.current.update(function(){
      return contents;
    });

    cb(err, self.current);
  }

  temp.get(resolvedFilepath, function(tempErr, tempContents){
    if(tempErr && tempErr.type !== 'NotFoundError'){
      return cb(tempErr);
    }

    if(tempContents){
      return handleContents(tempErr, tempContents);
    }

    fs.readFile(resolvedFilepath, 'utf8', handleContents);
  });
};

Workspace.prototype.saveFile = function saveFile(filepath, content, cb){
  var self = this;
  // TODO: make sure all arguments are passed

  if(typeof content.deref === 'function'){
    content = content.deref();
  }

  var directory = this._structure.cursor('directory');

  var cwd = this.cwd.deref();
  var dir = path.dirname(filepath);
  var filename = path.basename(filepath);

  var resolvedDir = path.resolve(cwd, dir);
  var resolvedFilepath = path.resolve(resolvedDir, filename);

  mkdirp(resolvedDir, function(mkdirpErr){
    if(mkdirpErr){
      return cb(mkdirpErr);
    }

    fs.writeFile(resolvedFilepath, content, 'utf8', function(writeErr){
      if(writeErr){
        return cb(writeErr);
      }

      function notContains(filename){
        return directory
          .filter(function(file){
            return file.name === filename;
          })
          .isEmpty();
      }

      if(notContains(filepath)){
        directory.update(function(list){
          return list.push({ name: filepath, temp: false });
        });
      }

      self.updateTemp(cb);
    });
  });
};

Workspace.prototype.deleteFile = function deleteFile(filepath, cb){
  var self = this;

  if(typeof filepath.deref === 'function'){
    filepath = filepath.deref();
  }

  var filename = this.filename;
  var current = this.current;
  var directory = this.directory;

  var cwd = this.cwd.deref();
  var resolvedFilepath = path.resolve(cwd, filepath);

  fs.unlink(resolvedFilepath, function(err){
    if(err){
      return cb(err);
    }

    filename.update(function(){
      return '';
    });

    current.update(function(){
      return '';
    });

    directory.update(function(list){
      return list.filter(function(file){
        return file.name !== filepath;
      });
    });

    temp.remove(resolvedFilepath, function(tempErr){
      if(tempErr){
        return cb(tempErr);
      }

      self._updateDirectory(cb);
    });
  });
};

Workspace.prototype.changeDir = function changeDir(dirpath, cb){
  var self = this;
  var cwd = this.cwd;
  var directory = this.directory;
  var projects = this.projects;

  // TODO: support more than root resolution
  var resolvedDir = path.resolve(root, dirpath);

  mkdirp(resolvedDir, function(mkdirpErr){
    if(mkdirpErr){
      return cb(mkdirpErr);
    }

    fs.readdir(root, function(projectsErr, directories){
      if(projectsErr){
        return cb(projectsErr);
      }

      projects.update(function(list){
        var empty = list.clear();
        return empty.concat(directories);
      });

      fs.readdir(resolvedDir, function(err, files){

        var fullPaths = files.map(function(file){
          return path.resolve(resolvedDir, file);
        });

        temp.exists(fullPaths, function(tempErr, tempExists){

          var fileObjs = files.map(function(file, idx){
            return { name: file, temp: tempExists[idx] };
          });

          directory.update(function(list){
            var empty = list.clear();
            return empty.concat(fileObjs);
          });

          cwd.update(function(){
            return resolvedDir;
          });

          cb(err, self.directory);
        });
      });
    });
  });
};

Workspace.prototype.deleteDir = function deleteDir(dirpath, cb){
  if(typeof dirpath.deref === 'function'){
    dirpath = dirpath.deref();
  }

  var projects = this.projects;
  // TODO: support more than root resolution
  var resolvedDir = path.resolve(root, dirpath);

  del(resolvedDir, function(err){
    if(err){
      return cb(err);
    }

    projects.update(function(list){
      return list.filter(function(project){
        return project !== dirpath;
      });
    });

    cb();
  });
};

Workspace.prototype.updateTemp = function updateTemp(cb){
  var self = this;
  var cwd = this.cwd.deref();
  var filepath = this.filename.deref();
  var current = this.current.deref();
  var resolvedFilepath = path.resolve(cwd, filepath);

  fs.readFile(resolvedFilepath, 'utf8', function(readErr, fileContents){
    if(current === fileContents){
      return temp.remove(resolvedFilepath, function(rmErr){
        if(rmErr){
          return cb(rmErr);
        }

        self._updateDirectory(cb);
      });
    }

    temp.add(resolvedFilepath, current, function(err){
      if(err){
        return cb(err);
      }

      self._updateDirectory(cb);
    });
  });
};

Workspace.prototype._updateDirectory = function _updateDirectory(cb){
  var cwd = this.cwd.deref();
  var directory = this.directory;
  var files = directory.toJS();

  var fullPaths = files.map(function(file){
    return path.resolve(cwd, file.name);
  });

  temp.exists(fullPaths, function(tempErr, tempExists){

    var fileObjs = files.map(function(file, idx){
      return { name: file.name, temp: tempExists[idx] };
    });

    directory.update(function(list){
      var empty = list.clear();
      return empty.concat(fileObjs);
    });

    if(typeof cb === 'function'){
      cb();
    }
  });
};

module.exports = Workspace;
